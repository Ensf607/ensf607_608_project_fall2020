...
//The server will use this class to decode your json
	...
	public String getResponse(String json) throws JsonMappingException, JsonProcessingException {
		...
		if (node.get("type").asText().equals("GET")) {
			...
			case "CLIENT": {
				if (node.get("scope").asText().equals("select")) {
					if (node.hasNonNull("ClientID")) {
						String g = t.getSearchResult("CLIENT", "ClientID", node.get("ClientID").asText());
						return g;
					}
				}
			}
		...
		} else if (node.get("type").asText().equals("POST")) {
			switch (node.get("table").asText()) {
			case "PURCHASE":
				t.insertIntoPURCHASE(node.get("ClientID").asText(), node.get("ToolID").asText(), "CURRENT_TIMESTAMP");
